...
public class AddCommentServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String verificationCodeInput = request
				.getParameter("verificationCodeInput");
		String commentContent = request.getParameter("commentContent");
		String nickname = request.getParameter("nickname");
		if (null == verificationCodeInput || "".equals(verificationCodeInput)) {
			request.setAttribute("errorMessage", "请输入验证码");
			request.getRequestDispatcher("getArticleServlet?id=" + articleId)
		...
		if (verificationCodeInput.equalsIgnoreCase(verificationCode)) {
			...
			Comment comment = new Comment(commentContent, articleId,
					new Date(), nickname);
			CommentService commentService = DAOFactory
					.getCommentServiceInstance();
