...
public class GetArticlesOfCategoryServlet extends HttpServlet
{
			...
			throws ServletException, IOException 
	{
		...
		int totalPages = articleService.getTotalPagesOfCategory(categoryId, pageUtil.getPageSize());
		...
		if(totalPages != 0)
		{
			...
			articlesList =  articleService.getArticlesOfCategory(categoryId, pageNum, pageUtil.getPageSize());
		}
		//totalPages = 0;
		else
		{
		...
		request.getRequestDispatcher("articlesOfCategory.jsp").forward(request, response);
	}
