...
public class ManageArticleArchivesAdminServlet extends HttpServlet
{
			...
			throws ServletException, IOException
	{
		...
		List<ArticleArchive> articleArchivesList = articleArchiveService.manageAllArticleArchives();
		request.setAttribute("articleArchivesList", articleArchivesList);
		
		request.getRequestDispatcher("articleArchivesManagement.jsp").forward(request, response);
	}
