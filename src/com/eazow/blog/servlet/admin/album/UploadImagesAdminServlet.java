			...
			try
			{
				...
				for(FileItem fileItem: fileItemList)
				{
					...
					else
					{
						...
						lowResolutionFileName = fileName.substring(0, fileName.indexOf(".")) 
							+ "_low" + fileName.substring(fileName.indexOf("."));
						url = "album/" + fileName;
						lowResolutionUrl = url.substring(0, url.indexOf(".")) + "_low" + fileName.substring(fileName.indexOf("."));
						//创建输出流
						...
						ImageUtil.createThumbnail(savePath+fileName, savePath+lowResolutionFileName, 200);
						
						//将图片url插入数据库
						ImageService imageService = DAOFactory.getImageServiceInstance();
						Image image = new Image(fileName, url, lowResolutionUrl, albumId);
