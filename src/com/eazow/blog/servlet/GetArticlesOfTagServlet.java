...
public class GetArticlesOfTagServlet extends HttpServlet
{
	@Override
			...
			throws ServletException, IOException
	{
		...
		int totalPages = articleService.getTotalPagesOfTag(tagId, pageUtil.getPageSize());
		...
		if(totalPages != 0)
		{
			...
			articlesList =  articleService.getArticlesOfTag(tagId, pageNum, pageUtil.getPageSize());
		}
		//totalPages = 0;
		else
		{
		...
		request.getRequestDispatcher("articlesOfTag.jsp").forward(request, response);
	}
