...
public class GetArticlesByArticleArchiveServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		int totalPages = articleService.getTotalPagesByArticleArchive(
				articleArchiveId, pageUtil.getPageSize());
		...
		if (totalPages != 0) {
			...
			articlesList = articleService.getArticlesByArticleArchive(
					articleArchiveId, pageNum, pageUtil.getPageSize());
		} else {
		...
		request.getRequestDispatcher("articlesByArticleArchive.jsp").forward(
				request, response);
	}
}
