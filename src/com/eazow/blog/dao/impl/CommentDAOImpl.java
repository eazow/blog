...
public class CommentDAOImpl implements CommentDAO {
	...
	public List<Comment> getCommentsOfArticle(int articleId) {
		...
		try {
			...
			while (dbConnector.rs.next()) {
				comment = new Comment(dbConnector.rs.getInt("id"),
						dbConnector.rs.getString("content"), dbConnector.rs
								.getInt("article_id"), dbConnector.rs
								.getTimestamp("comment_date"), dbConnector.rs
								.getString("nickname"));
	...
	public synchronized boolean updateComment(Comment comment) {
		...
		try {
			...
			dbConnector.pstmt.setString(1, comment.getContent());
			dbConnector.pstmt.setInt(2, comment.getArticleId());
			dbConnector.pstmt.setTimestamp(3, new Timestamp(comment
