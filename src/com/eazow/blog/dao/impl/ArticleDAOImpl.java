...
public class ArticleDAOImpl implements ArticleDAO
{
	...
	public synchronized int addArticle(Article article)
	{
		...
		try
		{
			...
			dbConnector.pstmt.setTimestamp(3, new Timestamp(article.getPostDate().getTime()));
	...
	public synchronized boolean addViewCount(int articleId)
	{
		...
		try 
		{
			...
			if(dbConnector.rs.next())
			{
				...
				viewCount++;
				sql = "update article set view_count = " + viewCount + " where id = " + articleId;
...
//		String sql = "select * from article order by id desc";
		...
		try
		{
			...
			{
				article = new Article(dbConnector.rs.getInt("id"), dbConnector.rs.getString("title"), 
						dbConnector.rs.getString("content"), dbConnector.rs.getTimestamp("post_date"), 
						dbConnector.rs.getInt("category_id"), dbConnector.rs.getInt("comments_num"),
						dbConnector.rs.getInt("view_count"));
	...
	public List<Article> getArticlesOfCategory(int categoryId, int pageNum, int pageSize)
	{
		...
		String sql = "select * from article where category_id = " + categoryId 
			+ " order by post_date desc limit " + startLocation + ", " + pageSize;
		try
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
			...
			String afterDate)
	{
		...
		String sql = "select * from article where post_date >= '" + beforeDate  
			+ "' and post_date < '" + afterDate + "' order by post_date desc";
		try
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
			...
			String afterDate, int pageNum, int pageSize)
	{
		...
		int startLocation = (pageNum-1) * pageSize;
		String sql = "select * from article where post_date >= '" + beforeDate  
			+ "' and post_date < '" + afterDate + "' order by post_date desc limit " + startLocation + ", " + pageSize;
		try
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
	...
	public synchronized void addCommentsNumOfArticle(int articleId)
	{
		...
		try 
		{
			...
			if(dbConnector.rs.next())
			{
				...
				commentsNum++;
				sql = "update article set comments_num = " + commentsNum + " where id = " + articleId;
	...
	public int getArticlesNumOfCategory(int categoryId)
	{
		int articlesNum = 0;
		DBConnector dbConnector = null;
		String sql = "select count(*) from article where category_id = " + categoryId;
		try 
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
	...
	{
		DBConnector dbConnector = null;
		String sql = "select count(*) from article where post_date >= '" + beforeDate  
	...
	public synchronized int moveArticlesToDefaultCategory(int originalCategoryId)
	{
		DBConnector dbConnector = null;
		int result = 0;
		String sql = "update article set category_id = 1 where category_id = " + originalCategoryId;
		try
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
			...
			String afterDate, int pageSize)
	{
		...
		int totalPages = 0;
		DBConnector dbConnector = null;
		String sql = "select count(*) from article where post_date >= '" + beforeDate + "' and post_date < '" + afterDate + "'" ;
		try 
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
	...
	public List<Article> getArticles(int pageNum, int pageSize)
	{
		...
		int startLocation = (pageNum-1) * pageSize;
		DBConnector dbConnector = null;
//		String sql = "select * from article order by id desc limit " + startLocation + ", " + pageSize;
		String sql = "select * from article order by post_date desc limit " + startLocation + ", " + pageSize;
		try 
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
	...
	{
		DBConnector dbConnector = null;
		String sql = "select * from article where title = '" + title + "'" + " order by id desc";
		Article article = null;
		try 
		{
			dbConnector = new DBConnector();
