...
public class CategoryDAOImpl implements CategoryDAO {
		...
		try {
			...
			while (dbConnector.rs.next()) {
				category = new Category(dbConnector.rs.getInt("id"),
						dbConnector.rs.getString("name"), dbConnector.rs
								.getInt("articles_num"));
	...
	public synchronized boolean addArticlesNumOfCategory(int categoryId) {
		DBConnector dbConnector = null;
		int result = 0;
		String sql = "select articles_num from category where id = "
				+ categoryId;
		int articlesNum = 0;
		try {
			dbConnector = new DBConnector();
			...
			if (dbConnector.rs.next()) {
				...
				articlesNum++;
				sql = "update category set articles_num = " + articlesNum
						+ " where id = " + categoryId;
	...
	public synchronized boolean updateCategory(Category category) {
		...
		String sql = "update category set name = '" + category.getName()
				+ "', articles_num = " + category.getArticlesNum()
				+ " where id = " + categoryId;
		try {
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
