...
public class ImageDAOImpl implements ImageDAO
{
	...
	public List<Image> getLatestImages()
	{
		...
		try
		{
			...
			{
				image = new Image(dbConnector.rs.getInt("id"), dbConnector.rs.getString("name"), 
						dbConnector.rs.getString("url"), dbConnector.rs.getString("low_resolution_url"),
						dbConnector.rs.getTimestamp("upload_date"), dbConnector.rs.getInt("album_id"));
	...
	public List<Image> getImagesOfAlbum(int albumId)
	{
		List<Image> imagesList = new ArrayList<Image>();
		DBConnector dbConnector = null;
		String sql = "select * from image where album_id = " + albumId + " order by id asc";
		try
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
	...
	public int getIndexLocationOfAlbum(int imageId)
	{
		...
		String sql = "select count(*) from image where id >= " + imageId + " and album_Id = "
			+ image.getAlbumId() + " order by id desc";
		DBConnector dbConnector = null;
		int index = 0;
		try
		{
	...
	public Image getNextImageOfAlbum(int imageId)
	{
		...
		DBConnector dbConnector = null;
		String sql = "select * from image where id < " + imageId + " and album_id = " 
			+ imageTemp.getAlbumId() + " order by id desc";
		try
		{
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
	...
	public Image getPreviousImageOfAlbum(int imageId)
	{
		...
		DBConnector dbConnector = null;
		String sql = "select * from image where id > " + imageId + " and album_id = " 
			+ imageTemp.getAlbumId() + " order by id asc";
