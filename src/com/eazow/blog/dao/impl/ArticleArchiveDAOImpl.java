...
public class ArticleArchiveDAOImpl implements ArticleArchiveDAO {
		...
		try {
			...
			while (dbConnector.rs.next()) {
				articleArchive = new ArticleArchive(
						dbConnector.rs.getInt("id"), dbConnector.rs
								.getInt("year"),
						dbConnector.rs.getInt("month"), dbConnector.rs
								.getInt("articles_num"));
	...
	public ArticleArchive getArticleArchive(int year, int month) {
		ArticleArchive articleArchive = null;
		DBConnector dbConnector = null;
		String sql = "select * from article_archive where year = " + year
				+ " and month = " + month;
		try {
			dbConnector = new DBConnector();
			dbConnector.stmt = dbConnector.conn.createStatement();
	...
	public synchronized boolean addArticlesNumOfArticleArchive(
			int articleArchiveId) {
		DBConnector dbConnector = null;
		int result = 0;
		String sql = "select articles_num from article_archive where id = "
				+ articleArchiveId;
		int articlesNum = 0;
		try {
			dbConnector = new DBConnector();
			...
			if (dbConnector.rs.next()) {
				...
				articlesNum++;
				sql = "update article_archive set articles_num = "
						+ articlesNum + " where id = " + articleArchiveId;
	...
	public synchronized boolean reduceArticlesNumOfArticleArchive(
			int articleArchiveId) {
		DBConnector dbConnector = null;
		int result = 0;
		String sql = "select articles_num from article_archive where id = "
	...
	public boolean updateArticleArchive(Article article) {
		...
		for (ArticleArchive articleArchive : articleArchiveList) {
			// 有对应的文章存档
			if (year == articleArchive.getYear()
					&& month == articleArchive.getMonth()) {
				return this.addArticlesNumOfArticleArchive(articleArchive
						.getId());
			}
