...
public class ArticleServiceImpl implements ArticleService {
	...
	private ArticleServiceImpl(ArticleDAO articleDAO, CategoryDAO categoryDAO,
			CommentDAO commentDAO, ArticleArchiveDAO articleArchiveDAO,
			TagDAO tagDAO, TagArticleDAO tagArticleDAO) {
	...
	public static ArticleService getArticleServiceInstance(
			ArticleDAO articleDAO, CategoryDAO categoryDAO,
			CommentDAO commentDAO, ArticleArchiveDAO articleArchiveDAO,
			TagDAO tagDAO, TagArticleDAO tagArticleDAO) {
		if (null == articleService)
			articleService = new ArticleServiceImpl(articleDAO, categoryDAO,
					commentDAO, articleArchiveDAO, tagDAO, tagArticleDAO);
	...
	public List<Article> getAllArticles() {
		...
		for (Article article : articlesList) {
			// Tags
			List<Integer> tagsIds = this.tagArticleDAO.getTagsIds(article
					.getId());
	...
	public List<Article> getArticlesOfCategory(int categoryId) {
		List<Article> articlesList = articleDAO
	...
	public List<Article> getArticlesOfCategory(int categoryId, int pageNum,
			int pageSize) {
		List<Article> articlesList = articleDAO.getArticlesOfCategory(
	...
	public boolean deleteArticle(int id) {
		...
		ArticleArchive articleArchive = this.articleArchiveDAO
				.getArticleArchive(year, month);
		// 减少文章存档对应的文章数
		this.articleArchiveDAO.reduceArticlesNumOfArticleArchive(articleArchive
				.getId());
		// 减少分类所对应的文章数
	...
	public int getTotalPagesByArticleArchive(int articleArchiveId, int pageSize) {
		ArticleArchive articleArchive = articleArchiveDAO
				.getArticleArchive(articleArchiveId);
		...
		return this.articleDAO.getTotalPagesByArticleArchive(beforeDateStr,
				afterDateStr, pageSize);
	}
	...
	public List<Article> getArticlesByArticleArchive(int articleArchiveId,
		...
		Date afterDate = calendar.getTime();
		String afterDateStr = simpleDateFormat.format(afterDate);
		List<Article> articlesList = articleDAO.getArticlesByArticleArchive(
				beforeDateStr, afterDateStr, pageNum, pageSize);
		for (Article article : articlesList) {
			// Tags
			List<Integer> tagsIds = this.tagArticleDAO.getTagsIds(article
					.getId());
	...
	public boolean updateArticle(Article article) {
		...
		if (yearOriginal != year || monthOriginal != month) {
			ArticleArchive articleArchive = this.articleArchiveDAO
					.getArticleArchive(yearOriginal, monthOriginal);
			this.articleArchiveDAO
					.reduceArticlesNumOfArticleArchive(articleArchive.getId());
			this.articleArchiveDAO.updateArticleArchive(article);
		}
		...
		// categoryId发生改变
			...
			this.categoryDAO.reduceArticlesNumOfCategory(articleOriginal
					.getCategoryId());
	...
	public List<Article> getArticlesOfTag(int tagId, int pageNum, int pageSize) {
		List<Integer> articlesIds = this.tagArticleDAO.getArticlesIds(tagId,
				pageNum, pageSize);
