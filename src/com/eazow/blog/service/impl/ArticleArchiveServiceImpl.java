...
public class ArticleArchiveServiceImpl implements ArticleArchiveService
{
	...
	public static ArticleArchiveService getArticleArchiveServiceInstance(ArticleArchiveDAO articleArchiveDAO,
			ArticleDAO articleDAO)
	{
		if(null == articleArchiveService)
			articleArchiveService = new ArticleArchiveServiceImpl(articleArchiveDAO, articleDAO);
		return articleArchiveService;
	}
	
	private ArticleArchiveServiceImpl(ArticleArchiveDAO articleArchiveDAO, ArticleDAO articleDAO)
	{
	...
	{
		List<ArticleArchive> articleArchivesList = this.articleArchiveDAO.getArticleArchiveList();
...
//		}
	...
	{
		ArticleArchive articleArchive = this.articleArchiveDAO.getArticleArchive(id);
		int year = articleArchive.getYear();
		int month = articleArchive.getMonth();
		String beforeDate = DateUtil.getFirstDayOfMonth(year, month);
		String afterDate = DateUtil.getFirstDayOfMonth(year, month+1);
		List<Article> articlesList = this.articleDAO.getArticlesByArticleArchive(beforeDate, afterDate);
	...
	public List<ArticleArchive> manageAllArticleArchives()
	{
		...
		for(ArticleArchive articleArchive: articleArchivesList)
		{
			...
			List<Article> articles = this.articleDAO.getArticlesByArticleArchive(beforeDateStr, afterDateStr);
