...
CKEDITOR.dialog.add("uicolor", function (a) {
	...
	function f(i) {
		if (/^#/.test(i)) {
			i = window.YAHOO.util.Color.hex2rgb(i.substr(1));
		}
		c.setValue(i, true);
		c.refresh("cke_uicolor_picker");
	}
	function g(i, j) {
		if (j || b._.contents.tab1.livePeview.getValue()) {
			a.setUiColor(i);
		}
	...
	d = {id:"yuiColorPicker", type:"html", html:"<div id='cke_uicolor_picker' style='width: 360px; height: 200px; position: relative;'></div>", onLoad:function (i) {
		...
		for (var l = 0; l < k.count(); l++) {
			k.getItem(l).addClass("cke_dialog_ui_input_text");
		}
	}};
	var h = true;
	...
	onShow:function ()
	 {
		...
		var i = a.getUiColor();
		if (i) {
			this.setValue("config.uiColor = \"" + i + "\"");
		}
